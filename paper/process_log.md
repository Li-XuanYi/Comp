...
## 节点 12：图表浮动体与页面留白再优化
- 针对用户指出的图8、表8和图15占页突兀问题，重新调整后半部分浮动体策略：压缩全局图表上下间距，取消问题二以后各大节的强制分页，使图表优先随正文自然流动。
- 缩小定价经济性图、车辆规模面板图、基地调度机制图、基地方案对比图和空间鲁棒性图的显示宽度，避免单幅图占满页面造成大面积留白。
- 将灵敏度分析表格设为更紧凑的局部字号和列间距，并对关键表图使用 `[!htbp]` 提高就近排版优先级。
- 检查：LaTeX 环境和图片引用结构检查通过；相关 pytest 6 项通过；`git diff --check` 仅提示 Windows 换行转换。本机仍缺少 XeLaTeX/TeX Live，最终视觉效果需在 Overleaf 使用 XeLaTeX 编译确认。
## 节点 13：恢复节点10排版并保留细节修复
- 按用户要求，将图表排版策略恢复到节点10：保留各大部分前分页、使用 `[H]` 控制图表就地出现，并恢复图8、图15及相关图表的节点10显示宽度。
- 保留节点11以来的数学符号与公式渲染修复，包括 `\hat{y}_{i,12}`、`\bar{\pi}_i`、`p_{k}^{ref}`、`\tau_{ji}` 等写法，以及独立公式末尾标点。
- 检查：LaTeX 环境和图片引用结构检查通过；异常文本残留检查通过；相关 pytest 6 项通过。本机仍缺少 XeLaTeX/TeX Live，PDF 需在 Overleaf 使用 XeLaTeX 编译确认。
